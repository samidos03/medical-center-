...
<head>
    ...
    <title>Book Appointment - Bahjawa Medical</title>
...
.hamburger{display:none;width:34px;height:34px;border-radius:9px;border:1px solid #e2e8f0;background:white;align-items:center;justify-content:center;cursor:pointer;color:#64748b;flex-shrink:0}
    ...
    @media(max-width:480px){.topbar p{display:none}}
    </style>
</head>
<body>
...
<div class="main" id="main">
    <div class="topbar">
        <div><h1>Book Appointment</h1><p>Schedule a new appointment</p></div>
    </div>
        ...
        <form method="POST" action="{{ route('patient.rendezvous.store') }}">
            ...
            <div class="form-card">
                ...
                <div class="field">
                    ...
                    @error('medecin_id')<p style="color:#dc2626;font-size:12px;margin-top:4px">{{ $message }}</p>@enderror
                </div>
                <div class="grid2">
                    <div class="field">
                        <label>Date</label>
                        <input type="date" name="date_rdv" value="{{ old('date_rdv') }}" required min="{{ date('Y-m-d') }}">
                        @error('date_rdv')<p style="color:#dc2626;font-size:12px;margin-top:4px">{{ $message }}</p>@enderror
                    </div>
                    <div class="field">
                        <label>Time</label>
                        <input type="time" name="heure_rdv" value="{{ old('heure_rdv','09:00') }}" required>
                        @error('heure_rdv')<p style="color:#dc2626;font-size:12px;margin-top:4px">{{ $message }}</p>@enderror
                    </div>
                </div>
            </div>
            <div style="display:flex;gap:10px;flex-wrap:wrap">
                <button type="submit" class="btn-primary" style="padding:12px 24px;font-size:14px">Book Appointment</button>

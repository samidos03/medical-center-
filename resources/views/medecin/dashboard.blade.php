...
<head>
    ...
    <title>Espace Médecin — Bahjawa Medical</title>
    @vite(['resources/css/app.css', 'resources/js/app.js'])
    <style>
    *{box-sizing:border-box;margin:0;padding:0}
    html,body{overflow-x:hidden}
    body{font-family:Segoe UI,system-ui,sans-serif;background:#f0fdf4;min-height:100vh;display:flex}
    .sidebar{width:260px;min-height:100vh;background:linear-gradient(180deg,#064e3b,#065f46,#059669);display:flex;flex-direction:column;position:fixed;top:0;left:0;bottom:0;z-index:50;transition:transform .3s ease}
    ...
    .main{flex:1;margin-left:260px;min-height:100vh;display:flex;flex-direction:column;min-width:0}
    ...
    .stat-card{background:white;border-radius:16px;padding:18px;border:1px solid #e2e8f0;box-shadow:0 1px 3px rgba(0,0,0,.06)}
    ...
    .stat-num{font-size:26px;font-weight:700;color:#0f172a;margin-bottom:4px}
    ...
    .pt{background:#f8fafc;color:#64748b}
    .btn-primary{display:inline-flex;align-items:center;gap:6px;background:linear-gradient(135deg,#059669,#064e3b);color:white;border:none;border-radius:10px;padding:9px 18px;font-size:13px;font-weight:600;cursor:pointer;text-decoration:none}
    .btn-confirm{display:inline-flex;align-items:center;gap:5px;background:#f0fdf4;color:#059669;border:1px solid #bbf7d0;border-radius:8px;padding:6px 12px;font-size:12px;font-weight:600;cursor:pointer}
    .btn-cancel{display:inline-flex;align-items:center;gap:5px;background:#fef2f2;color:#dc2626;border:1px solid #fecaca;border-radius:8px;padding:6px 12px;font-size:12px;font-weight:600;cursor:pointer}
    .hamburger{display:none;position:fixed;top:14px;left:14px;z-index:100;background:white;border:1px solid #e2e8f0;border-radius:10px;padding:8px;cursor:pointer;align-items:center;justify-content:center}
        ...
        <div class="logo-inner">
            <div class="logo-icon">
                <svg width="28" height="28" viewBox="0 0 40 40" fill="none">
                    <circle cx="20" cy="20" r="14" fill="#f0fdf4" stroke="#059669" stroke-width="1.5"/>
                    <path d="M20 10v20M10 20h20" stroke="#059669" stroke-width="2.5" stroke-linecap="round"/>
                </svg>
            </div>
            <div>
...
<div class="main" id="main">
    <div class="topbar">
        <div>
            ...
            <p>{{ now()->format('d/m/Y') }} — Tableau de bord médecin</p>
        </div>
    </div>
    ...
    <div class="content">
        ...
        @if(session('success'))
            <div style="background:#f0fdf4;border:1px solid #bbf7d0;border-radius:12px;padding:12px 16px;color:#16a34a;font-size:13px;margin-bottom:20px">
                {{ session('success') }}
            </div>
        @endif
        ...
        <div class="stats-grid">
            ...
            <div class="stat-card">
                ...
                <div class="stat-num">{{ $stats['confirmes'] }}</div>
        ...
        <div class="card">
            <div class="card-header">
                ...
                <a href="{{ route('medecin.rendezvous.index') }}" class="btn-primary">Voir tout</a>
            </div>
            <table>
                <thead>
                    <tr>
                    ...
                    <tr>
                        <td data-label="Patient" style="font-weight:600">
                            <div style="display:flex;align-items:center;gap:8px">
                                <div style="width:30px;height:30px;background:#eff6ff;border-radius:50%;display:flex;align-items:center;justify-content:center;font-weight:700;font-size:11px;color:#0369a1;flex-shrink:0">
                                    {{ strtoupper(substr($rdv->patient->user->name??'P',0,2)) }}
                                </div>
                                {{ $rdv->patient->user->name??'—' }}
                            </div>
                        </td>
                        <td data-label="Heure" style="font-weight:700;color:#059669">{{ substr($rdv->heure_rdv,0,5) }}</td>
                        <td data-label="Motif" style="color:#64748b">{{ $rdv->motif??'—' }}</td>
                        <td data-label="Statut">
                            ...
                            @else<span class="pill pt">Terminé</span>@endif
                        </td>
                        <td data-label="Actions">
                            <div style="display:flex;justify-content:flex-end;gap:6px;flex-wrap:wrap">
                                ...
                                <form action="{{ route('medecin.rendezvous.annuler', $rdv) }}" method="POST" onsubmit="return confirm('Annuler ce RDV ?')">
                                    @csrf @method('PATCH')
                                    <button type="submit" class="btn-cancel">Annuler</button>
                                </form>
                                @endif
                                <a href="{{ route('medecin.consultations.create') }}" class="btn-primary" style="padding:6px 12px;font-size:12px">
                                    Consulter
                                </a>
                            </div>
                        </td>
